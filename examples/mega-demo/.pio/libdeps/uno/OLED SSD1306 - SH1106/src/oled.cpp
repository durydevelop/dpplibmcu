...
//__attribute__((deprecated("Deprecated, use printf_XY() instead")))
...
size_t OLED::printf_XY(uint_fast8_t x, uint_fast8_t y, const char *format, ...) {
    va_list arg;
    va_start(arg, format);
    char temp[64];
    char* buffer = temp;
...
size_t OLED::printf_T(uint_fast8_t col, uint_fast8_t row, const char *format, ...) {
    va_list arg;
    va_start(arg, format);
    char temp[64];
    char* buffer = temp;
